...
void initMap() {
    ...
    kMap.push_back(make_pair("NULL_", NULL_));


    oMap.clear();
    oMap.push_back(make_pair("EQUAL", EQUAL));
    oMap.push_back(make_pair("GRT_THAN", GRT_THAN));
    oMap.push_back(make_pair("LES_THAN", LES_THAN));
    oMap.push_back(make_pair("GRT_EQUAL", GRT_EQUAL));
    oMap.push_back(make_pair("LES_EQUAL", LES_EQUAL));
    oMap.push_back(make_pair("NOT_EQUAL", NOT_EQUAL));
    oMap.push_back(make_pair("NOT_EQUAL_", NOT_EQUAL_));
    oMap.push_back(make_pair("AND", AND));
    oMap.push_back(make_pair("AND_", AND_));
    oMap.push_back(make_pair("OR", OR));
    oMap.push_back(make_pair("OR_", OR_));
    oMap.push_back(make_pair("XOR", XOR));
    oMap.push_back(make_pair("POINT", POINT));

    sMap.clear();
    sMap.push_back(make_pair("LEFT_BRA", LEFT_BRA));
    sMap.push_back(make_pair("RIGHT_BRA", RIGHT_BRA));
    sMap.push_back(make_pair("COMMA", COMMA));
...
void dealBegin(string &name, TokenType &type){
    ...
    else if(isalpha(*iter) || *iter=='_') { cout<<"INTO IDN"<<endl; state = State::IN_IDN;}
...
void dealIDN(string &name, TokenType &type){
    ...
    } else if((seekKey(name) != IDN) && seekKey(name)<30) {
        state = State::IN_KW;
        cout << "INTO KW1" << endl;
    } else {
        type = TokenType::IDN;
...
void printTokenLink() {
    ...
    for(auto token : tokens) {
        string s(token.name);
        cout <<token.name<<"    "<<"<"<<TokenDict[token.type]<<", "<< seekKey(s) <<">"<<endl;
    }
